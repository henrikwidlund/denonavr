...
from denonavr.foundation import AvrCommandError, DenonAVRDeviceInfo
...
class TestDenonAVRDeviceInfo:
    ...
    async def test_async_tactile_transducer_on_returns_early_when_on(self):
        ...
        with patch.object(
            device.telnet_api.__class__, "connected", new=property(lambda self: False)
        ), patch.object(
            device.telnet_api.__class__, "healthy", new=property(lambda self: False)
        ), patch.object(
            api, "async_get_command", new_callable=AsyncMock
        ) as mock_get_command:
    ...
    @pytest.mark.asyncio
    async def test_async_hdmi_audio_decode_returns_early_when_matches(self):
    ...
    @pytest.mark.asyncio
    async def test_async_video_processing_mode_returns_early_when_matches(self):
